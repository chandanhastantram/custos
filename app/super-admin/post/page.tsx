...
export default function PostsPage() {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'gallery': return 'bg-blue-600/20 text-blue-500';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button 
          onClick={() => alert('➕ Create New Post\n\nSelect post type:\n• Announcement\n• Event\n• Notice\n• Gallery\n\nThen add title, content, and attachments.')}
                ...
                <tr key={post.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      <button 
                        onClick={() => alert(`✏️ Edit Post: "${post.title}"\n\nThis will open the post editor.`)}
                        className="p-2 rounded hover:bg-muted transition-colors"
                      >
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => confirm(`🗑️ Delete "${post.title}"?\n\nThis action cannot be undone.`) && alert('Post deleted!')}
                        className="p-2 rounded hover:bg-muted text-red-400 transition-colors"
                      >
